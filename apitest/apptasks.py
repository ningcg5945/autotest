...
def appauto_testcase(self):  # app例
    ...
    time.sleep(1)
    sql = "SELECT id,appfindmethod,appevelement,appoptmethod,appassertdata,`apptestresult` from apptest_appcasestep " \
          "where apptest_appcasestep.Appcase_id=1 ORDER BY id ASC "
...
def appauto_testcase2(self):  # 流程 的 相关接口
    ...
    time.sleep(1)
    sql = "SELECT id,appfindmethod,appevelement,appoptmethod,appassertdata,`apptestresult` from apptest_appcasestep " \
          "where apptest_appcasestep.Appcase_id=2 ORDER BY id ASC "
    coon = pymysql.connect(user='root', passwd='test123456', db='autotest', port=3306, host='127.0.0.1', charset='utf8')
    cursor = coon.cursor()
    aa = cursor.execute(sql)
    info = cursor.fetchmany(aa)
