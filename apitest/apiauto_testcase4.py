...
def writeResult(case_id, result):
    ...
    sql = "UPDATE apitest_apistep set apitest_apistep.apistatus=%s,apitest_apistep.create_time=%s where " \
...
def caseWriteResult(case_id, result):
    result = result.encode('utf-8')
    now = time.strftime("%Y-%m-%d %H:%M:%S")
    sql = "UPDATE apitest_apitest set apitest_apitest.apitestresult=%s,apitest_apitest.create_time=%s where " \
          "apitest_apitest.id = % s; "
    param = (result, now, case_id)
    print('api autotest result is ' + result.decode())
    coon = pymysql.connect(user='root', passwd='test123456', db='autotest', port=3306, host='127.0.0.1', charset='utf8')
    cursor = coon.cursor()
...
def writeBug(bug_id, interface_name, request, response, res_check):
    ...
    sql = "INSERT INTO `bug_bug` (" \
          ...
          "VALUES('%s', '%s', '1', '1', 'Ning', 'Ning', '%s', '2');" % (bugname, pymysql.escape_string(bugdetail), now)
    coon = pymysql.connect(user='root', passwd='test123456', db='autotest', port=3306, host='127.0.0.1', charset='utf8')
    cursor = coon.cursor()
    cursor.execute(sql)
    coon.commit()
...
if __name__ == '__main__':
    ...
    filename = "D:\\Program Files\\Python36-32\\Scripts\\autotest\\apitest\\templates\\" + 'apitest_report' + '{}'.format(nowTime) + ".html"
    fp = open(filename, 'wb')
